  ...
  cors({
    origin: function (origin, callback) {
      ...
      if (allowedOrigins.indexOf(origin) === -1) {
        var securityMessage = "Why CORS policy does not allow access?";
        return callback(new Error(securityMessage), false);
      }
...
const alertMe = new EventEmitter();

function alert(message) {
  console.log("!!   alert   !! " + message);
}

function newAlert(alertName) {
  console.log("+  alertName   +");
  alert(alertName + " triggered !");
}

newAlert("Healthcheck_Failed");

alertMe.on("alertName", newAlert);

function login(alertName) {
  alertMe.emit("alertName", alertName);
}
...
let listDevicesConnected = () =>
  SerialPort.list().then((ports) =>
    ports.forEach((port) => DevicesConnected.push(port.path))
  );
...
const FanRelay = new Gpio(16, 'out');       
let stopFan = false;

const TaggleRelay = _ => {
  if (stopFan) {
    return FanRelay.unexport();
  }
 
  FanRelay.read()
    .then(value => FanRelay.write(value ^ 1))
    .then(_ => setTimeout(TaggleRelay, 1000))
    .catch(err => console.log(err));
};
 
TaggleRelay();
 
setTimeout(_ => stopFan = true, 50000);
*/
...
app.listen(
  ...
  function () {
    ...
    console.log(
      "Server Started on http://localhost:" + SERVER_PORT + "/app/name"
    );
    logger.log({
      level: "info",
